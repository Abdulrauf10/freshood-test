...
import useAddStoreMine from "./hooks/useAddStoreMine"
...
const EditStoreInformation: React.FC = () => {
  ...
  const {
    ...
    watch,
    mutation
  } = useAddStoreMine(dataProductMine?.data?.image.id)
      ...
      <Box as="form" method="POST">
        <HStack gap={{ base: 0, md: 8 }} mb={4}>
          ...
          <HStack width="80%" justifyContent="center">
            <CustomTitle title="Edit Store Info" />
          </HStack>
        </HStack>
        ...
        <VStack gap={4} alignItems={"flex-start"}>
          ...
          <Button
            ...
            onClick={() =>
              onSubmit({
                name: watch("name"),
                image_id: dataProductMine?.data?.image.id || 0
              })
            }
          >
            Save
