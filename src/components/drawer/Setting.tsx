...
const MenuItem: React.FC<MenuItemProps> = ({ imageSrc, altText, text, onClick }) => (
    <HStack cursor={"pointer"} onClick={onClick} width="100%" justifyContent={"space-between"}>
        <HStack>
        ...
        <>
            ...
            {menuSections.map((section, index) => (
                <VStack key={index} width="100%" spacing={8} mt={8}>
                    <HStack width="100%">
                        <Box as="span" color={"#1B1917"} fontWeight={"600"} whiteSpace="nowrap"
                            >{section.title}</Box>
                        <Divider mt={2} width={"100%"} />
                    </HStack>
                    <VStack width="100%" spacing={8}>
                        {section.items.map((item, index) => (
                            <MenuItem onClick={() => handleDrawer(item.name)} key={index} {...item} />
                        ))}
                    </VStack>
                </VStack>
            ))}
            ...
            <VStack spacing={4} alignItems={"start"}>
                ...
                <Text cursor={"pointer"} color={"#B91C1C"}>Delete account</Text>
            </VStack>
        </>
    )
}
