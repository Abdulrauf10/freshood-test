...
import { Box, Button, HStack, Heading, Input, Text, VStack, IconButton, useToast, Flex, InputGroup, InputLeftElement, Progress, Divider } from '@chakra-ui/react';
...
const MessagePage: React.FC = () => {
    ...
    const sendMessage = (channelUrl: string) => {
        ...
        if (channel) {
            channel.sendUserMessage(message, '', '', function (message: any, error: any) {
                if (error) {
                    console.log(error);
                    return;
                }
                ...
                <Flex direction={"column"} gap={4}>
                    ...
                    <Flex maxW={'768px'} flexWrap={"wrap"} gap={2}>
                        {typeChat.map((type, index) => (
                            <Button borderWidth={1} backgroundColor={
                                selectedTypeChat === type ? '#016748' : 'white'
                            } key={index} borderRadius={"xl"} color={
                                selectedTypeChat === type ? 'white' : 'black'
                            } onClick={
                                () => setSelectedTypeChat(type)
                            }>{type}</Button>
                        ))}
                    </Flex>
                    <Flex p={4} borderWidth={1} borderRadius={"xl"}>
                        <VStack alignItems={"start"}>
                            <HStack>
                                ...
                                <Text color={"#44403C"} fontWeight={"600"}>Reaching Chatroom Quota</Text>
                            </HStack>
                            <Text color={"#78716C"}>You are reaching quota. You can still receive messages, but unable to view clients' names and profile pictures.</Text>
                            <HStack minW="100%" gap={4}>
                                <Progress value={80} w={"80%"} colorScheme='gray' borderRadius={"md"} />
                    ...
                    <VStack alignItems={"start"} gap={4}>
                        {channels.map((channel, index) => {
                            const flatColors = ['#1abc9c', '#2ecc71', '#3498db', '#9b59b6', '#34495e', '#f1c40f', '#e67e22', '#e74c3c', '#ecf0f1', '#95a5a6'];
                            return (
                                <>
                                    <HStack key={channel.url} cursor={"pointer"} justifyContent={"space-between"} onClick={() => selectChannel(channel)}>
                                        <HStack>
                                            <Box w="50px" h="50px" borderRadius={"full"} backgroundColor={flatColors[index]} />
                                            <VStack pl={4} alignItems={"start"}>
                                                <Text color={"#78716C"} fontSize={"11px"}>Brand Name</Text>
                                                <Text>{channel.lastMessage ? channel.lastMessage?.message : ''}</Text>
                                            </VStack>
                                        </HStack>
                                        <VStack alignItems={"end"}>
                                            <Text>
                                                {channel.lastMessage ? new Date(channel.lastMessage.createdAt).toLocaleDateString() : ''}
                                            </Text>
                                            <Box w="30px" h="30px" borderRadius={"full"} backgroundColor={'#016748'}>
                                                <Text color={"white"} textAlign="center" pt={1}>{unreadMessages}</Text>
                                            </Box>
                                        </VStack>
